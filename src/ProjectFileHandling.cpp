...
Sequentially (no additional line breaks between the data):
    ...
    Number of additional options - 32 bit int
    For each Option:
            Length of the option as a string in bytes - int
            The option itself - see writing function in NodeComponents::Option
...
int64_t project_file_handling::readLong(std::istream* readFrom){
...
int32_t project_file_handling::readInt(std::istream* readFrom){
...
std::string* project_file_handling::readString(std::istream* readFrom){
...
void project_file_handling::loadProjectFromString(std::string* content, GraphEditor* editor, ProjectProperties* properties){
    ...
    std::string* preDescription = readString(from);
    ...
    delete preName;
    delete preDescription;
    delete preAnswers;

    int optionAmount = readInt(from);
    for(int optionNumber = 0; optionNumber < optionAmount; optionNumber++){
        std::string* optionStr = readString(from);
        std::istringstream optionStream = std::istringstream(*optionStr);
        node_components::Option* opt = new node_components::Option(&optionStream);
        properties->extraOptions.push_back(opt);
    }
...
void project_file_handling::writeLong(std::ostream* writeTo, int64_t data){
    writeTo->write((char*)(&data), sizeof(int64_t) / sizeof(char));
}

void project_file_handling::writeInt(std::ostream* writeTo, int32_t data){
    writeTo->write((char*)(&data), sizeof(int32_t) / sizeof(char));
}

void project_file_handling::writeString(std::ostream* writeTo, std::string* data){
...
void project_file_handling::saveProject(std::string path, GraphEditor* editor, ProjectProperties* properties){
    ...
    std::string desc = "TASK DESCRIPTION NOT YET IMPLEMENTED";
    writeString(to, &desc);
    writeString(to, &properties->answers);

    writeInt(to, properties->extraOptions.size());
    for(node_components::Option* opt : properties->extraOptions){
        std::ostringstream optionStream;
        opt->write(&optionStream);
